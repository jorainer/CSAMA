...
... slightly more complicated than for other omics.

- m/z is not the mass.
- assuming `[M+H]+` and `[M+Na]+` ions we could match m/z against reference
  values.
- `MetaboAnnotation`: `matchValues()`: need here an example from the
  MetaboAnnotationTutorials?
  

## Annotation

- consider also retention time.


## Annotation

- What with the unknowns?
- MS2 spectra: how are fragment spectra created?

## Annotation

- `MetaboAnnotation` `matchSpectra`.
- ... still a lot to do. Comprehensive high quality reference
  databases. Annotation software.
